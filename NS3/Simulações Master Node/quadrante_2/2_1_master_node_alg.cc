...
//Configurações da rede
    ...
    int vet[nRn][1] = {rn, 19, 24, 13, 29, 23};
    // Vetor com clientes previamente instalados
    int cli[nCon][1] = {27, 26, 25, 23, 22, 21, 18, 17, 16, 3, 1, 0, 6, 9, 13, 14, 15, 29, 5, 10, 12};
    // Vetor com Retransmissores previamente instalados
    int ser[nCon][1] = {19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 24, 24, 24, 24, 24, 24, 13, 13, 13};
...
//Obter posição de arquivo .txt
  ...
  arqx = fopen("Master_Node/coordenadas/quadrante_2/2_1_50_pos_x.txt", "rt"); 
  arqy = fopen("Master_Node/coordenadas/quadrante_2/2_1_50_pos_y.txt", "rt");
...
// Instalação dos Relay Nodes (RN)
  ...
  for(int p = 0; p<nAll; ++p)
  {
        ...
        if(first == false){
          ...
          if(entra == true){
              if(p != rn && p != 19 && p != 24 && p != 13 && p != 29 && p != 23)
              {
...
//Gerar animação
        AnimationInterface anim ("Master_Node/algoritmo/2_1_" + gp + "_master_node_alg.xml"); // Mandatory
        ...
        anim.EnableIpv4RouteTracking ("Master_Node/algoritmo/2_1_" + gp + "_master_node_alg_route.xml", Seconds (0), Seconds (5), Seconds (0.25)); //Optional
...
// Obter Média da Simulação
 ...
 double DesJitter = 0;
  ...
  std::cout << "Nó Selecionado "<< vet[0][0] <<std::endl;
    ...
    std::ofstream myfile ("Master_Node/algoritmo/2_1_"+gp+"_Results_Algoritmo.csv");
 ...
 if (myfile.is_open())
  ...
  std::ofstream con ("Master_Node/algoritmo/2_1_"+gp+"_Conectados_Alg.csv");
